...
routeraddress=$(netstat -rn |awk 'NR==5{print $1}')
routername=$(getent hosts $(netstat -rn $(netstat a |awk '/: e/{gsub(/:/,"");print $2}')|awk '/inet /{gsub(/\/.*/,"");print $2}')| awk 'NR==3{print $2}')
externalIP=$(curl -s icanhazip.com)
externalHostname=$(getent hosts $externalIP | awk '{print $2}')
cat <<EOF
Hostname        : $(hostname)
LAN Address     : $routeraddress
LAN Hostname    : $routername
