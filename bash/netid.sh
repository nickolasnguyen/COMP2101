...
declare -A interface
interface[interface$index]=$(ip a s | awk -F'[: ]+' 'NR==7 {print$2}')
[ "$verbose" = "yes" ] && echo "Reporting on interface(s):${interface[interface$index]}"

[ "$verbose" = "yes" ] && echo "Getting IPV4 address and name for interface ${interface[interface$index]}"
# Find an address and hostname for the interface being summarized
# we are assuming there is only one IPV4 address assigned to this interface
ipv4_address=$(ip a s ${interface[interface$index]}|awk -F '[/ ]+' '/inet /{print $3}')
ipv4_hostname=$(getent hosts $ipv4_address | awk '{print $2}')

[ "$verbose" = "yes" ] && echo "Getting IPV4 network block info and name for interface ${interface[interface$index]}"
...
network_address=$(ip route list dev ${interface[interface$index]} scope link|cut -d ' ' -f 1)
